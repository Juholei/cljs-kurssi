...
(defmethod navigate :product-details [{params :params}]
  (.log js/console "Valitaan tuote: " (pr-str params))
  (products/select-category-by-name! (:category params))
  (products/set-selected-product-id! (js/parseInt (:id params)))
  (products/set-page! :product-page))

(defmethod navigate :cart []
  (products/set-page! :cart))

(defmethod navigate :checkout []
  (products/set-page! :checkout))
